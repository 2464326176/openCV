 ...
 * @Date: 2022/8/4 22:48
 * @Description: 
 * @FilePath: region_of_interest.cpp
 */
...
//    waitKey(0);
    Point2i p2i;
    p2i.x = img.cols / 2 - mask.cols / 2;
    p2i.y = img.rows / 2 - mask.rows / 2;
    Mat imageROI = img(Rect(p2i.x, p2i.y, mask.cols, mask.rows));
//    Mat imageROI = img(Range(p2i.y, p2i.y + mask.rows), Range(p2i.x, p2i.x + mask.cols));
